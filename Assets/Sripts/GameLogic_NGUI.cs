...
public class GameLogic_NGUI : MonoBehaviour 
{
	...
	void Start () 
	{
		...
		UIWidget widget = (UIWidget)GridPrefabList[0].GetComponent<UIWidget>();
		GridHeight = widget.height / 100.0f;
		GridWidth = widget.width / 100.0f;
	...
	void AddRandomGrid(int X, int Y)
	{
		...
		float PosX, PosY;
		PosX = BottomPoint.position.x + X * GridWidth;
		PosY = BottomPoint.position.y + Y * GridHeight - GridHeight;
		GridList[X, Y]._grid.transform.position = new Vector3(PosX, PosY, 0.0f);
		...
		GridList [X, Y]._grid.transform.localScale = new Vector3 (1.0f, 1.0f, 1.0f);
		GridList[X, Y]._grid.name += "["+X+","+Y+"]";
		GridList[X, Y].IsEmpty = false;

		Debug.Log ("AddRandomGrid x=" + X + " y=" + Y + " PosX" + PosX + " PosY" + PosY);
